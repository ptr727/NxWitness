...
The `version` (major.minor) in [version.json](./version.json) is the version floor; NBGV computes the project `SemVer2` (`X.Y.<height>`, with a `-g<sha>` prerelease suffix off non-`main` refs). That NBGV version is the **GitHub release tag** on `main` and the `LABEL_VERSION` build arg baked into the images - distinct from the Docker image *tags*, which carry the Nx product version from `Make/Matrix.json`. **`develop` leads `main` by a minor:** after a `develop -> main` release lands and main's publish completes, bump the minor in `version.json` on `develop` in an isolated `bump-version-X.Y` PR, so develop's NBGV prereleases sort above main's last stable (at a shared `X.Y`, develop's prerelease `X.Y.<height>-g<sha>` sorts *below* main's stable `X.Y.<height>`). A **maintenance** `develop -> main` promotion (dependency bumps, CI/doc fixes, template re-syncs) holds main's version - `git checkout main -- version.json` on the promotion branch - so `main` advances only its height, not its minor.
