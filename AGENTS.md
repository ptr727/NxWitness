...
- **Docker Hub readme repositories derived from `Matrix.json`.** `publish-docker-readme-task.yml` is carried verbatim (generic: `repositories` input, `ref` gate, optional transform). Because the image set is repo-specific, the `publish-release.yml` caller computes the `repositories` list from `main`'s `Matrix.json` (lowercased `ptr727/<image>` plus the shared base repos) in a `get-repos` job and passes it in.
...
- **Build-layer action pins stay repo-owned.** The orchestration-layer workflows pin every shared action to the template's SHA. The repo-owned build-layer leaves (`build-docker-task.yml`, `build-base-images-task.yml`, `test-release-task.yml`) own their `docker/*` and other action versions and track them via Dependabot.
