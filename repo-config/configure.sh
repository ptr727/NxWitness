...
cmd_check() {
  ...
  # access to the per-product + shared-base Docker Hub repos. Verify it by hand in the Docker Hub account.
  # The repo list is derived from Make/Matrix.json: docker.io/ptr727/<image> for each product image plus the
  # shared bases nx-base + nx-base-lsio (currently: dwspectrum[-lsio], nxgo[-lsio], nxmeta[-lsio],
  # nxwitness[-lsio], wisenetwave[-lsio], nx-base, nx-base-lsio).
  note "verify manually: Docker Hub access token valid with push to docker.io/ptr727/<images from Make/Matrix.json> + nx-base + nx-base-lsio"
