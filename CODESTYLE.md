...
- **AOT is opt-in.** `<PublishAot>false</PublishAot>` is the default in `CreateMatrix.csproj`, and publishing with `-p:PublishAot=true` builds an AOT binary. `IsAotCompatible` holds for every build, while `VerifyReferenceAotCompatibility` applies only to an AOT publish, where its `IL3058` warnings stay warnings rather than errors. Several dependencies are not built as AOT-compatible, among them `System.CommandLine`, the Serilog packages, and the assemblies `Microsoft.Extensions.Http.Resilience` brings in, so verifying them on every build, or failing on them, would break the build.
